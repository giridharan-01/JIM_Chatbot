...
# ---------------- QUESTION SELECTION (FIXED WITH RERUN) ---------------- #
for key, idx in section_map.items():
        ...
        for unit in selected_units:
            ...
            for i, q in enumerate(questions):
                ...
                if checkbox_key not in st.session_state:
                    st.session_state[checkbox_key] = q in selected_list

                # ✅ Lock logic
                disabled = (
                    not st.session_state[checkbox_key]
                    and len(selected_list) >= limit
                )
                ...
                # ---------------- SAFE UPDATE ---------------- #
                if checked != (q in selected_list):

                    # ✅ FULL REBUILD BEFORE RERUN (CRITICAL FIX)
                    updated_list = []
                    ...
                    if len(updated_list) <= limit:
                        ...
                        st.session_state.selected_questions[key] = updated_list
                        count = len(updated_list)

                        # ✅ SHOW TOAST ONLY WHEN LIMIT IS REACHED (ONCE)
                        if count == limit and not st.session_state.get(f"{key}_toast_shown", False):
                            st.toast(f"✅ {name_map[key]} limit reached ({limit})", icon="✅")
                            st.session_state[f"{key}_toast_shown"] = True

                        # 🔄 Reset if user unchecks
                        elif count < limit:
                            st.session_state[f"{key}_toast_shown"] = False

                    else:
                        # ❌ HARD BLOCK
                        st.session_state[checkbox_key] = False
                        st.toast(f"⚠️ Max limit is {limit}", icon="⚠️")
